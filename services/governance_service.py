...
def _load_quality_summary(project_name):
    ...
    try:
        ...
        return {
            ...
            "dead_code_summary": _extract_dead_code_summary(data.get("issues", [])),
        }
    except (json.JSONDecodeError, OSError):
        return None


def _extract_dead_code_summary(issues):
    from services.dead_code_signal import extract_dead_code_summary
    return extract_dead_code_summary(issues)
...
def apply_rule_to_project(project_name, reason, rule_text):
    from services.governance_rules_service import apply_rule_to_project as _impl
    return _impl(project_name, reason, rule_text)


def get_rule_effectiveness(project_name):
    from services.governance_rules_service import get_rule_effectiveness as _impl
    return _impl(project_name)


def generate_policy_snippets(project_name):
    from services.governance_rules_service import generate_policy_snippets as _impl
    return _impl(project_name)
