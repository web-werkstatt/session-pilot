"""
SPEC-AUDIT-ANALYZER-LLM-001: LLM-basierter Audit-Analyzer via Perplexity.
Opt-in via AUDIT_LLM_ANALYZER_ENABLED=1. Aendert niemals AuditResult.status,
ergaenzt nur evidence["llm_review"] oder evidence["llm_review_error"].
"""
import json

from config import AUDIT_LLM_ANALYZER_ENABLED, AUDIT_LLM_MAX_REQUIREMENTS
...
def run_analyzers(spec, results, input_facts):
    ...
    Wenn AUDIT_LLM_ANALYZER_ENABLED=0: No-Op, gibt results unveraendert zurueck.
    Wenn enabled: analysiert max AUDIT_LLM_MAX_REQUIREMENTS Requirements.
    """
    if not AUDIT_LLM_ANALYZER_ENABLED:
        return results

    changed_files = input_facts.get("changed_files", [])
    limit = min(len(results), AUDIT_LLM_MAX_REQUIREMENTS)
