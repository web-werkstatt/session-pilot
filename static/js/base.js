...
function _setMobileSidebar(open) {
    ...
    if (open) {
        var firstNavItem = sidebar.querySelector('.nav-item');
        if (firstNavItem) firstNavItem.focus();
    } else if (_lastSidebarTrigger && typeof _lastSidebarTrigger.focus === 'function') {
        _lastSidebarTrigger.focus();
    }
}

function closeMobileSidebar() {
    _setMobileSidebar(false);
}
...
function toggleSidebar() {
    ...
    if (_isMobileSidebar()) {
        _lastSidebarTrigger = document.activeElement;
        _setMobileSidebar(!sidebar.classList.contains('mobile-open'));
        return;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.sidebar .nav-item').forEach(function(item) {
        item.addEventListener('click', function() {
            if (_isMobileSidebar()) closeMobileSidebar();
        });
    });
});
...
/* Lightbox: ausgelagert nach lightbox.js (Phase 7, 2026-04-14).
   getLightbox() bleibt nur als Referenz im Escape-Handler weiter unten benutzt. */
...
function updateStickyPositions() {
    ...
    // Phase 7 (2026-04-14): nur theads mit position:sticky anfassen — vermeidet tote
    // style="top:..." auf Settings-Tabellen und anderen statischen Tables.
    document.querySelectorAll('thead').forEach(function(thead) {
        var table = thead.closest('table');
        if (!table || table.offsetParent === null) return;
        if (getComputedStyle(thead).position !== 'sticky') return;
...
(function() {
    ...
    function updateStickyStack() {
        ...
        // Phase 7 (2026-04-14): nur sticky-th anfassen — keine toten inline-tops.
