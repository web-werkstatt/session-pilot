...
- **2026-04-09:** Monorepo-Detailseiten weiter gestrafft: fuer Projekte wie `/project/auth-stack` werden generische Auto-Sections mit wenig Aussagekraft jetzt unterdrueckt oder nachrangig behandelt. `Structure` und `Root Orientation` stehen vorne, waehrend triviale `Description`-, `Size`- und kleine `Code Statistics`-Bloecke fuer Monorepos nicht mehr die Ansicht aufblaehen (`routes/project_info_routes.py`).
- **2026-04-09:** Projekt-Detailseiten um eine echte Arbeitsansicht erweitert: neuer Block `Was kann ich verbessern?` zeigt jetzt `Was ist gut`, `Was fehlt` und `Was als naechstes`, abgeleitet aus README, Doku, Tests, CI, Projektbeschreibung und vorhandenem Quality-Scan. Damit beantwortet `/project/<name>` nicht mehr nur `Was ist da?`, sondern auch `Was fehlt und wo fange ich an?` (`routes/project_info_routes.py`).
- **2026-04-09:** Projekt-Detailseiten weiter entruempelt: fuer Monorepos ist der generische `Details`-Block entfernt, `Root Orientation` wurde zu kompakteren `Quick Entry Points` reduziert. Damit bleiben `Was kann ich verbessern?`, `Structure` und ein brauchbares `README` klar im Vordergrund (`routes/project_info_routes.py`).
- **2026-04-09:** Projekt-Detailseite rendert den separaten `README`-Block jetzt nur noch, wenn aus `/api/info` wirklich ein README geliefert wird. Projekte ohne README wie `auth-stack` zeigen dadurch keinen sinnlosen `README`-Eintrag mehr in `Contents` (`static/js/project-detail.js`).
- **2026-04-09:** `Contents`-Sidebar auf der Projekt-Detailseite entfernt. Die Overview-Ansicht ist jetzt wieder einspaltig und zeigt nur noch die eigentlichen Inhaltsbloecke ohne rechte TOC-Navigation (`templates/project_detail.html`, `static/css/project-detail.css`, `static/js/project-detail.js`).
- **2026-04-09:** `Contents`-Sidebar auf Wunsch wiederhergestellt. Die Projekt-Detailseite hat damit erneut die rechte TOC-Navigation mit Scroll-Links auf die sichtbaren Sections (`templates/project_detail.html`, `static/css/project-detail.css`, `static/js/project-detail.js`).
- **2026-04-09:** Planning fuer Subprojekte verbessert: der Projekt-Planning-API-Endpunkt faellt jetzt bei `project_type = subproject` automatisch auf `parent_project` zurueck. Der Planning-Tab zeigt dadurch z. B. bei `feed-monitor` das Parent-Planning statt leerer Inhalte und markiert im UI, dass das Workspace-Planning vom Parent kommt (`services/plan_structure_service.py`, `routes/plans_routes.py`, `static/js/project-planning.js`).
- **2026-04-09:** Empty-State im Projekt-Planning fuer Subprojekte praezisiert: wenn ein Subprojekt das Parent-Planning erbt, aber auch dort noch keine Plaene existieren, sagt die Leermeldung das jetzt explizit statt irrefuehrend `No plans assigned to this project yet` zu zeigen (`static/js/project-planning.js`).
- **2026-04-09:** Subprojekt-Planning-Fallback korrigiert: Subprojekte nutzen jetzt zuerst eigene Plan-Eintraege. Nur wenn keine eigenen `project_plans` existieren, wird auf `parent_project` zurueckgefallen. Dadurch zeigt `feed-monitor` seinen eigenen Plan statt trotz vorhandener Plan-Datei leer zu bleiben (`services/plan_structure_service.py`).
- **2026-04-09:** Projekt-Detailseite um einen sichtbaren Fortschrittsblock erweitert. `Details` zeigt jetzt oberhalb von `Was kann ich verbessern?` eine `Fortschritt`-Section mit Planning-Readiness, Progress-Bar und Kennzahlen fuer Plaene, Sprints, Specs und Tasks. Die Werte werden direkt aus dem Planning-System abgeleitet und markieren bei Subprojekten auch die Quelle des Planning (`routes/project_info_routes.py`).
